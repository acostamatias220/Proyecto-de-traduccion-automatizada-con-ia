...
*/
..//SetComic
function SetComic($COMIC_X,$COMIC_Y,$マンガ記号,$何個目){
...
function ComicProcess(){
	...
	if($マンガ記号==1){
		...
		$Comic_Name4_1 = "@"+$Comic_Name1;
		$Comic_Name4_2 = "@"+$Comic_Name2;
		...
		$Comic_Name7 = "@"+$Comic_Name1;

		Fade($Comic_Name7, 300, 1000, Dxl1, false);
		Zoom($Comic_Name7, 300, 1000, 1000, Dxl1, false);

		Wait(250);

		Shake($Comic_Name7, 300, 0, 1, 0, 0, 500, null, true);

	}else if($マンガ記号==8){


		$Comic_Name8 = "@"+$Comic_Name1;
...
/*========================
	...
	SetFont("ＭＳ 明朝", 15, FFFFFF, 000000, MEDIUM, RIGHTDOWN);
...
function EffectZoom(EF画像優先度, EF所要時間, SHAKE所要時間, $EF画像データ, EF待ち)
{
	...
	FadeDelete("function攻撃汎用100", EF所要時間, null, EF待ち);
...
$フラッシュ優先度 = $EF画像優先度+1;
	...
	FadeDelete("functionEF色100", EF所要時間, null, false);
	FadeDelete("function攻撃汎用100", EF所要時間, null, EF待ち);
...
..//新生LastfireEX
function LastfireEX($LastfireEXNutName)
{
...
..//TurboBlur
function TurboBlur($効果ナット名)
{
	CreateProcessEX("ProTurboBlur", "TurboBlurAct");
	Request("ProTurboBlur", Start);
}


function TurboBlurAct()
{
	while(1){
